...
    
    // el.dataset.pieceId = id; // REMOVE/IGNORE: This ID is 'p0', 'p1', not the DSL ID.
    el.dataset.slot = key;
    el.style.backgroundColor = PIECE_COLORS[type] || '#999';
    
    // --- CRITICAL FIX START ---
    // 1. Look up the correct DSL ID (1, 2, 3...) from the global config based on the target slot (r_c)
    const solutionData = window.currentGameConfig?.solutionMap?.[key];
    
    if (solutionData && solutionData.dslId) {
        // 2. Add the attribute that puzzleGuide.js is looking for!
        el.setAttribute('data-piece-id', solutionData.dslId); // ✅ THIS IS THE FIX
